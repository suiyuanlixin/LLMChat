...
| `auto_compact.max_chars` | 触发自动压缩的历史字符数阈值，使用字符数粗略估算上下文大小。 |
...
上下文压缩使用“压缩摘要 + 最近消息窗口”的策略。自动压缩开启后，普通聊天和 Agent 模式都会在当前历史字符数超过 `auto_compact.max_chars` 时，将较早消息和已有压缩摘要交给 `auto_compact.compact_model` 生成新的连续性摘要，并保留最近 `auto_compact.keep_recent_messages` 条原始消息。
