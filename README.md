...
- 支持三类 API：`glm` 使用 ZAI SDK，`anthropic` 使用 Anthropic SDK，`openai` 使用 OpenAI SDK。
- 支持自定义 `base_url`，可接入 Anthropic Messages API 或 OpenAI Chat Completions 兼容端点。
...
pip install rich zai-sdk anthropic openai
```
...
| 字段 | 说明 |
| --- | --- |
| `api_type` | API 类型，只支持 `glm`、`anthropic` 或 `openai`。 |
| `base_url` | 自定义 API 地址。`glm` 类型会自动忽略该字段；`anthropic` 和 `openai` 类型可留空使用 SDK 默认地址，也可填写兼容端点。MiniMax OpenAI 兼容接口填写 `https://api.minimaxi.com/v1`。 |
| `model` | 模型名称，按服务商要求填写。 |
...
Anthropic Agent 模式使用 streaming API 读取工具调用事件和 thinking delta，避免长请求触发 SDK 的非流式限制。GLM / OpenAI Agent 模式使用 Chat Completions 工具循环，并会在每轮响应返回后立即显示可用的 thinking。
