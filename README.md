...
LLMChat 是一个基于 Python 和 Rich 的终端大模型聊天客户端。它支持 GLM / ZhipuAI、Anthropic Messages API、OpenAI Chat Completions API、Gemini API 以及 Ollama 本地 / 云端模型，并提供会话保存、流式输出、推理内容展示和本地文件 agent 模式。
...
- 支持会话上下文自动压缩和 `/comp` 手动压缩，使用分层摘要 + 最近消息窗口延续长对话。
- 支持 Tavily 网络搜索，普通聊天可按需自动检索，Agent 模式会注入 `web_search` 工具。
- 支持持久记忆、偏好记忆、情景记忆和热历史，记忆更新可使用独立模型。
- 支持本地文件 agent 模式，可让模型在限定工作目录内读文件、搜索代码、编辑文件和执行命令。
- 支持 Agent todo 计划审批、状态恢复和最终校验，适合多步骤本地任务。
...
| `context_window_tokens` | 当前聊天模型的上下文窗口 token 数，用于按比例计算自动压缩阈值。不同模型请按服务商文档填写。 |
...
| `skills.sources.workspace` | 是否加载工作目录 `.llmchat/skills/`，默认关闭；开启后如果目录不存在会自动创建。 |
...
| `auto_compact.enable` | 是否启用自动上下文压缩。开启后普通聊天和 Agent 模式都会在上下文超过阈值时自动压缩。 |
...
| `auto_compact.compact_model` | 执行压缩摘要的模型。留空时使用当前聊天模型；填写模型名时统一使用该模型做自动和手动压缩。 |
| `memory_system.memory_model` | 执行持久记忆和情景记忆更新的模型。留空时使用当前聊天模型。 |
| `web_search.enable` | 是否开启网络搜索功能。开启且配置 Tavily key 后，普通聊天和 Agent 都可以使用搜索。 |
...
首次运行时会自动创建 `prompt.md`，可以在其中写入自定义提示词、人格、回复风格和个人偏好。程序会在每次模型请求前读取 `prompt.md`，普通聊天和 Agent 模式都会生效。
...
网络搜索使用 Tavily。开启并配置 key 后，普通聊天会在需要近期、外部或容易变化的信息时让模型调用搜索；Agent 模式也会获得 `web_search` 工具。搜索结果会包含来源 URL，回答需要引用搜索结果中的链接。
...
当前 provider 只支持 `tavily`。`search_depth` 支持 `basic`、`fast`、`ultra-fast`、`advanced`，`topic` 支持 `general`、`news`、`finance`。如果未配置 `web_search.api_key`，`/search` 会显示 `missing key`，普通聊天和 Agent 都不会真正执行网络搜索。
...
| 工具 | 功能 |
| --- | --- |
| `update_todos` | 维护当前 Agent todo 计划，支持优先级、依赖、完成标准、blocked/failed 原因和校验状态。 |
...
/plan reject
/plan retry <todo-id>
/plan unblock <todo-id>
/plan history
/plan clear
```

多步骤任务中，Agent 会先用 `update_todos` 创建计划。`agent_mode.approve=confirm` 时，执行非计划工具前会要求确认；`auto` 会自动批准工作目录内的低风险流程。计划和事件日志保存在 `<workspace>/.llmchat/plans/`，该目录已被 `.gitignore` 忽略。
...
Agent skills 只在 agent 模式中可用，支持两个来源：程序目录 `skills/` 和工作目录 `.llmchat/skills/`。工作目录来源默认关闭，开启后如果目录不存在会自动创建。每个 skill 使用一个目录，目录名只能包含小写字母、数字、`-` 和 `_`：
...
默认安装到工作目录 `.llmchat/skills/`；使用 `--app` 会安装到程序目录 `skills/`，对所有开启程序目录来源的工作区生效。安装前会展示预览并要求确认，可用 `--dry-run` 只预览、`--force` 覆盖已有 skill、`--yes` 跳过确认、`--version <version>` 指定版本、`--registry <url>` 指定 registry。安装来源和 lock 文件分别写入 `.clawhub/` 或 `.skillhub/`，这些目录已被 `.gitignore` 忽略。
...
| `/plan` | 查看当前 Agent todo 计划、审批状态、预算和计划文件路径。 |
| `/plan check` | 查看当前 todo 质量检查结果。 |
| `/plan approve [note]` / `/plan reject [reason]` | 手动批准或拒绝当前 Agent 计划。 |
| `/plan retry <todo-id>` / `/plan unblock <todo-id>` | 将 failed 或 blocked 的 todo 恢复为可继续执行。 |
| `/plan history [limit]` / `/plan clear` | 查看计划事件日志或清空当前 todo。 |
...
| `/skills workspace on` / `/skills workspace off` | 开启或关闭工作目录 `.llmchat/skills/` 来源，开启时目录不存在会自动创建。 |
...
上下文压缩使用“压缩摘要 + 最近消息窗口”的策略。自动压缩开启后，普通聊天和 Agent 模式都会根据上一轮模型调用返回的输入 token 判断是否超过 `context_window_tokens * auto_compact.trigger_ratio`，并用当前历史的本地 token 粗估补上本轮新增消息。超过阈值时，将较早消息和已有压缩摘要交给 `auto_compact.compact_model` 生成新的连续性摘要，并保留最近 `auto_compact.keep_recent_messages` 条原始消息。如果服务端没有返回 usage，客户端会临时使用本地 token 粗估作为兜底。

`auto_compact.compact_model` 留空时使用当前聊天模型；填写模型名时，自动压缩和 `/comp` 手动压缩都会统一使用该模型。手动执行 `/comp` 时，如果当前消息数不超过 `keep_recent_messages`，会提示取消压缩。

持久记忆和情景记忆更新使用 `memory_system.memory_model`。该字段留空时使用当前聊天模型；填写后只影响记忆写入，不影响压缩摘要模型。
...
├── planning.py              # Agent todo 计划、审批状态、事件日志和质量检查
