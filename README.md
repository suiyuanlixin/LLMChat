...
LLMChat 是一个基于 Python 和 Rich 的终端大模型聊天客户端。它支持 GLM / ZhipuAI、Anthropic Messages API、OpenAI Chat Completions API 以及 Ollama 本地 / 云端模型，并提供会话保存、流式输出、推理内容展示和本地文件 agent 模式。

## 功能亮点

- 支持四类 API：`glm` 使用 ZAI SDK，`anthropic` 使用 Anthropic SDK，`openai` 使用 OpenAI SDK，`ollama` 使用 Ollama Python SDK。
- 支持自定义 `base_url`，可接入 Anthropic Messages API、OpenAI Chat Completions 兼容端点或 Ollama 服务端。
- 支持普通输出与流式输出，流式模式会实时打印模型返回内容。
- 支持展示模型返回的推理 / 思考内容，例如 GLM `reasoning_content`、Anthropic thinking block 或 MiniMax OpenAI 兼容接口的 `reasoning_details`。
...
`requirements.txt` 中包含运行所需的完整 Python 依赖；如果只想手动安装核心包，可安装 `rich zai-sdk anthropic openai ollama`。
...
| 字段 | 说明 |
| --- | --- |
| `api_type` | API 类型，只支持 `glm`、`anthropic`、`openai` 或 `ollama`。 |
| `base_url` | 自定义 API 地址。`glm` 类型会自动忽略该字段；`anthropic` 和 `openai` 类型可留空使用 SDK 默认地址，也可填写兼容端点。MiniMax OpenAI 兼容接口填写 `https://api.minimaxi.com/v1`。`ollama` 留空时使用本地默认服务，通常是 `http://localhost:11434`；直连 Ollama 云端时填写 `https://ollama.com`。 |
| `model` | 模型名称，按服务商要求填写。 |
| `api_key` | API Key。Ollama 本地模型和通过本地 Ollama 代理调用云端模型可留空；直连 `https://ollama.com` 时填写 Ollama API Key，也可以使用 `OLLAMA_API_KEY` 环境变量。请只保存在本地，不要提交到仓库。 |
...
流式模式会实时显示模型返回的 thinking delta 和 text delta。实际是否能看到推理内容取决于模型和服务商是否返回对应字段。Ollama thinking 模型会读取 `message.thinking` 字段；`gpt-oss` 系列开启 thinking 时默认使用 `medium` 级别。

Anthropic Agent 模式使用 streaming API 读取工具调用事件和 thinking delta，避免长请求触发 SDK 的非流式限制。GLM / OpenAI Agent 模式使用 Chat Completions 工具循环，Ollama Agent 模式使用 `/api/chat` 工具调用循环，并会在每轮响应返回后立即显示可用的 thinking。
