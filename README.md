...
| `compaction.enable` | 是否启用自动上下文压缩。开启后普通聊天和 Agent 模式都会在上下文超过阈值时自动压缩。 |
| `compaction.max_chars` | 触发自动压缩的历史字符数阈值，使用字符数粗略估算上下文大小。 |
| `compaction.keep_recent_messages` | 压缩时保留最近多少条原始消息，其余更早消息会折叠进压缩摘要。 |
| `compaction.compact_model` | 执行压缩摘要的模型。留空时使用当前聊天模型；填写模型名时统一使用该模型做自动和手动压缩。 |
...
| `/comp` | 立即按当前 compaction 配置压缩会话上下文；消息数不超过 `keep_recent_messages` 时会取消压缩。 |
| `/agent` | 查看当前 agent 状态。 |
...
上下文压缩使用“压缩摘要 + 最近消息窗口”的策略。自动压缩开启后，普通聊天和 Agent 模式都会在当前历史字符数超过 `compaction.max_chars` 时，将较早消息和已有压缩摘要交给 `compaction.compact_model` 生成新的连续性摘要，并保留最近 `compaction.keep_recent_messages` 条原始消息。

`compaction.compact_model` 留空时使用当前聊天模型；填写模型名时，自动压缩和 `/comp` 手动压缩都会统一使用该模型。手动执行 `/comp` 时，如果当前消息数不超过 `keep_recent_messages`，会提示取消压缩。
