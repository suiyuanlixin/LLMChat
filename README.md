...
- 支持 agent skills，可在程序目录 `skills/` 下放置本机专用工作流指令。
...
| `agent_mode.skills` | 是否启用 agent skills。只读取程序目录下的 `skills/`，不会读取工作目录里的 skills。 |
...
| `list_skills` | 列出程序目录 `skills/` 下可用的 agent skills。 |
...
Agent skills 只在 agent 模式中可用，并且只从程序目录 `skills/` 读取。每个 skill 使用一个目录，目录名只能包含小写字母、数字、`-` 和 `_`：
...
Skills 只提供工作流指导，不会执行 skill 里的脚本，也不能覆盖 agent 安全规则、工作目录限制、审批设置或工具限制。`skills/` 已被 `.gitignore` 忽略，适合放本机专用指令。
...
| `/agent skills on` / `/agent skills off` / `/agent skills reload` | 开启、关闭或重新加载 agent skills。 |
| `Ctrl+C` | 中断并退出程序。 |
