...
| `max_agent_rounds` | 每次用户请求中，agent 最多执行多少轮工具循环。 |
| `max_agent_tool_calls` | 每次用户请求中，agent 最多调用多少次工具。 |
| `agent_approval_mode` | Agent 写操作审批模式，支持 `confirm` 或 `auto`。默认 `confirm`。 |
...
- 所有工具只能访问工作目录及其子目录。
- 路径中不允许使用父级目录引用。
- 写入类工具会展示 unified diff。`agent_approval_mode=auto` 时会自动批准工作目录内的文件编辑和低风险命令。
...
| `/agent approve confirm|auto` | 设置 Agent 写操作审批模式。 |
| `Ctrl+C` | 中断并退出程序。 |
...
Recent agent mode improvements:

- Adds an agent-specific system prompt so the model explores before editing, keeps changes small, verifies when safe, and summarizes the result.
- Hides noisy round/tool/final-check status lines while keeping required confirmations, warnings and errors visible.
- Adds `list_dir` for directory exploration.
- Extends `read_file` with `start_line`, `end_line`, and optional line-number output.
- Adds `apply_patch` for confirmed line-range edits with unified-diff previews.
- Adds `apply_unified_patch` for contextual unified-diff edits.
- Adds read-only `git_status` and `git_diff` tools so the agent can inspect real workspace changes without shelling out.
- Tracks files changed during an agent run and automatically feeds a final `git diff --check`, `git status --short`, and `git diff --stat` summary back to the model before it can finalize.
- Streams Anthropic agent tool-use and thinking responses, folds completed agent history into a concise summary, compacts oversized tool results, and adds `/agent approve confirm|auto`.
- Uses green backgrounds for added diff lines, red backgrounds for removed diff lines, and no background for context lines.
- Normalizes spacing between thinking output, confirmations, warnings, command approvals and final responses.
- Adds configurable agent budgets: 12 tool rounds and 40 tool calls per user request by default.
- Adds `/agent stop` and treats `Ctrl+C` during an agent run as "stop agent" instead of immediately exiting the chat.
- Splits `bash` command handling into allowed, confirmation-required, and blocked command categories.
