...
流式模式会实时显示模型返回的 thinking delta 和 text delta。实际是否能看到推理内容取决于模型和服务商是否返回对应字段。Gemini 在 `thinking_mode=true` 时会请求 thought summary；Ollama thinking 模型会读取 `message.thinking` 字段；`gpt-oss` 系列开启 thinking 时默认使用 `medium` 级别。
